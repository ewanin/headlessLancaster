    ...
    return (
        <div className='bg-[#bfdbfe]'>
            ...
            <div className='md:mx-[5%] ml-[5%]'>
                {newData.headData[0].D.map((item) => (
                    <div key={item.id}>
                        <div className='text-[30px] text-[#1e293b] font-bold mb-4'>{item.name}</div>
                    </div>
                ))}

                <div className=' flex flex-wrap lg:justify-normal md:justify-normal justify-center'>
                    {
                        filteredPostsD.map((post) => (
                            <div className='mr-6 mb-10 w-fit' key={''}>

                                {/* Box */}
                                {newData.detailsData.map((item) => (
                                    <div key={item.id}>

                                        <div className=' xl:w-[300px] w-[280px] h-[360px] border border-[#71a4f1] bg-white rounded-tr-[50px] hover:shadow-[-8.0px_8.0px_1px_rgb(62,124,219)] bg-no-repeat bg-bottom bg-clouds'>
                                            {/* Icon */}
